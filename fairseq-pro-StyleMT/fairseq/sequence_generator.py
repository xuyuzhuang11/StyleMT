    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                ...
                '''expert_out = model.expert.decoder.forward(
                    tokens,
                    encoder_out=None,
                    incremental_state=None,
                )
                antiexpert_out = model.antiexpert.decoder.forward(
                    tokens,
                    encoder_out=None,
                    incremental_state=None,
                )'''
                # print(decoder_out[0].shape)
                # print(expert_out[0][:, -1, :].shape)
                # print(antiexpert_out[0][:, -1, :].shape)
            else:
            ...
            '''alpha = torch.tensor([0.2]).to("cuda")
            decoder_out_tuple[0][:, 0, :] = decoder_out_tuple[0][:, 0, :] + alpha * (expert_out[0][:, 0, :] - antiexpert_out[0][:, 0, :])'''
