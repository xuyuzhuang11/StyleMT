...
def train(args, trainer, task, epoch_itr):
    ...
    for i, samples in enumerate(progress):
        with metrics.aggregate("train_inner"), torch.autograd.profiler.record_function(
            ...
            if "transformer_retrieval" == getattr(args, "arch", None):
                inter_dropout = False if getattr(args, "inter_dropout_alpha", 0.0) == 0.0 else True
                intra_dropout = False if getattr(args, "intra_dropout_beta", 0.0) == 0.0 else True
                log_output = trainer.train_step(samples, inter_dropout=inter_dropout, intra_dropout=intra_dropout)
            else:
